...
- 2026-07-25: The public Remote Friend Code page now restores guest-facing reconnect context from browser storage: friend code, player name, and compact character draft only. This reduces refresh/reconnect friction without introducing stored host authority secrets. Verification: `npm run test:friend-relay`, `npm run test:engine`, `npm run test:all`, and `npm run build` passed.
