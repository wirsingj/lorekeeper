...
yaiml: 0.2
role: sot
title: LoreKeeper State Of The Table
purpose: Current product state, engineering direction, priorities, risks, and useful recent lessons for LoreKeeper.
belongs-here: current product direction, verified capabilities, active trust risks, UX priorities, working checklist, playtest lessons, near-term context.
not-here: durable architecture boundaries, command reference, exhaustive history, implementation details that no longer affect current work.
durability: volatile; synthesize and prune aggressively.
read-with: LoreKeeper Architecture; LoreKeeper Maintainer Guide.
update-when: direction, verified behavior, risks, priorities, acceptance criteria, or material lessons change.
agent-guidance: Preserve human product decisions. Verify implementation claims. Mark uncertainty. Remove resolved active risks instead of appending a diary.
---
...
The party members are the people around the table. The host is one of those party members, plus the software-side table owner for setup, invites, provider access, and recovery decisions. The DM at the table is the configured DM model/provider operating inside app-owned rails. The app owns continuity, state, rules, recovery, and table flow; the provider supplies narration, NPC behavior, atmosphere, suggested checks, and structured proposals.
...
- Provider/model setup belongs in Settings as app-level configuration, with Ollama install guidance and model choice under Models rather than as a front-door play mode.
...
7. Add UI under Friends And Seats: Local LAN Link, Remote Friend Code, Copy Browser Link, Copy Friend Code, Regenerate, Stop Sharing, waiting guests, and clear "experimental remote sharing" status.
...
- Remote share off-state copy is more demo-safe: Friends And Seats now tells the host to click Start Remote Sharing and says where the browser link/friend code will appear, instead of showing "not configured" language that looked like a setup failure.
...
- Remote browser direct links are less mysterious: the public page now checks the linked friend code before submit and tells the guest whether the host is connected or needs to click Reconnect Sharing.
...
- Remote browser seated mobile layout is less cramped: the Adventure/Party rail now participates in normal page scrolling instead of clipping the moment panel and party cards behind a small internal scroll.
- Remote browser guests now fill a compact character draft before asking to join; the relay carries that draft into the host's normal waiting-room authority so Remote Friend Code, LAN `/guest`, pre-table, and active-table seating all preserve the same player-in-a-chair intent.
- Remote relay snapshots are now size-bounded before leaving the host app, so normal story growth should not trigger Worker `payload_too_large` rejection. The browser guest receives a compact recent table window; the full campaign/story log remains host-local.
- Remote browser Table Talk now renders a local pending echo immediately after send and reconciles it against the next host snapshot, so slow relay/host round trips do not make a friend's chat message look like it disappeared.
...
- Remote relay host ownership is tighter: once a host connects a friend-code room, later host reconnects for that code must present the same unshared internal host token, so a guest who only knows the friend code cannot replace the host socket.
...
- Table Talk refreshes while the DM model is generating so side chat does not appear to vanish until the DM response posts.
...
- DM and AI companion Nudge are gated by table phase, guest/client role, recovery state, and combat active actor; host-visible Nudge buttons stay clickable in safe blocked states so they can explain what is needed without starting generation or triggering recovery.
...
- Remote relay page visual smoke now has an opt-in harness: `npm run smoke:relay-page` verifies the live guest page health, nonce CSP, code preload, and desktop/mobile screenshots under `data/runtime/ui-flow-artifacts/live-relay-page-*`.
...
- Copy changes can create trust debt when labels drift from ownership reality. Host is the software-side owner and party member; the configured DM model/provider is the DM at the table.

## State Invariant Matrix

| State | Allowed actions | Forbidden actions | Hidden/disabled expectations |
| --- | --- | --- | --- |
| App front door | Continue Table, Create New Table, Join Table, open Settings, delete selected visible campaign. | Provider turns, guest actions, combat changes, hidden starter campaign selection/deletion. | Table rails, notes, command input, diagnostics, model setup, and last-table controls stay hidden until a flow is chosen. |
...
| Adventure draft / opening ready | Host may Start Adventure, invite/seat guests, edit party/notes where safe, use Table Talk; host Nudge buttons may be clicked only to explain the opening gate. | DM Nudge provider calls, Send Turn, AI companion Nudge provider calls, guest action/pass/vote, debug submit, combat advancement. | Composer disabled with Start Adventure copy; Start Adventure visible once per current table session; Nudge explains the block without generating; duplicate Start clicks do not call provider twice. |
...
1. Front Door: improved, now centered on Continue Table, Create New Table, Join Table, and compact Settings.
...
7. Settings: still one dialog internally. Keep app/model setup in Settings, table sharing in Friends And Seats, and diagnostics/recovery focused behind explicit troubleshooting or recovery entry points.
...
8. Character creation is standardized across campaign creation, post-start host creation, browser guest join, and in-app Join Table.
...
14. The unified front door now treats Continue Table, Create New Table, and Join Table as the three primary flows, with model setup living under Settings.
...
133. Settings now opens calmer App and Models tabs from the front door, with local-table tools, diagnostics, and DM recovery hidden unless the relevant table/troubleshooting surface is chosen.
134. Settings entry points are now intent-aware: the front-door gear opens app/model setup, and the in-table Friends control opens Friends And Seats instead of app-level options.
...
136. The front door now gives primary weight to Continue Table, Create New Table, and Join Table, with Settings as a compact utility gear instead of a fourth play mode.
...
143. Settings now keeps app/model setup together for first-run configuration, while in-table Friends And Seats shows only seating/share controls and diagnostics/recovery stay behind explicit troubleshooting or recovery entry points.
...
149. The front door now presents Continue Table, Create New Table, and Join Table as the three first-class launcher choices, with app/model Settings behind a compact gear.
...
181. Hidden provider-status accessibility copy now uses model-source language instead of "Provider/manual bridge," closing one more player-facing naming leak without changing internal provider ids.
...
185. AI companion Nudge is now gated before the first Start Adventure opening, for guests, and during non-active combat turns through `table-action-controller.js`; UI chaos now verifies a blocked pre-opening companion Nudge cannot start generation or trigger recovery.
...
190. The front door now uses Continue Table, Create New Table, Join Table, and compact Settings language, while model configuration lives under Settings > Models with Ollama guidance.
191. Model and Friends setup copy no longer exposes "AI" tabs, Guest Page labels, provider prompts, provider bridge, manual fallback, or campaign-chat status wording in normal live paths; visible host status now uses model-source, Guest Lobby, and handoff language.
192. Visual audit caught model setup leaking into Friends And Seats because host chrome refreshes were unhiding whole settings panels after the settings-surface projection ran. Host chrome no longer overrides scoped settings-panel visibility, and the UI harness asserts Model Setup stays hidden inside Friends And Seats.
...
195. Settings surfaces are scoped by intent at runtime: the front-door Settings surface exposes App and Models tabs, while Friends And Seats, Diagnostics, and DM Recovery remain focused entry points; the hidden UI harness can still open diagnostics directly for chaos/audit coverage.
...
226. Model setup projection now lives in `app/provider-settings-controller.js`: provider defaults, campaign-creation model fallback, Ollama status labels, setup hints, model option labels, and selected-model chips are tested outside `app/app.js`.
...
233. Model Setup now shows a tested first-run Ollama readiness checklist: install Ollama, start Ollama, download the selected model, then test it. The checklist is projected from `app/provider-settings-controller.js` instead of renderer-owned copy, and the portable release smoke command verifies the packaged app can serve `/guest` from a temp copy.
234. Failed local-provider turns that produce no reviewed DM text now get a visible recovery path: Try Again and Details appear, opening failures release the Start Adventure latch, the composer no longer contradicts recovery state, and DM Recovery explains that the DM could not answer instead of showing an idle panel.
235. Host-side Nudge buttons now stay clickable in safe blocked states, including pre-opening and failed-response recovery, so the host gets a table-facing explanation without accidentally starting a model turn. The UI harness verifies the click does not generate, mutate, or enter recovery.
236. Failed-response DM Recovery now offers an explicit Open Model Setup action, with the action plan owned by `host-response-review-controller.js`; the renderer only switches the existing Settings dialog to the Models tab so hosts can check Ollama/model readiness without hunting through app settings after a 404 or missing-model error.
...
29. Remote Friend Code has a built-in alpha relay and first-pass browser guest table, but it is not release-calm yet. Browser close now sends guest disconnect through the relay and the host app cleans up through the local authority route. The public browser page preserves guest-facing reconnect context locally: friend code, player name, character draft, and last seated chair as a preferred-seat hint, but not authority secrets. Friends And Seats can regenerate the remote friend code to invalidate a stale/leaked relay room, and browser-initiated relay requests are pinned to the current active friend code before the host bridge touches local guest authority. The remaining risk is live soak and hardening: durable resume identity after browser close, richer expiry/idle UI, fuller async handler no-mutation tests, and real off-LAN Electron-host plus browser-guest play.
30. Combat tracker no longer advances enemy turns from the general post-turn/local-table polling recovery loop. Enemy auto-resolution is limited to explicit post-import automation so the bottom-left combat tracker cannot silently flip through initiative while nobody is being prompted.
...
1. Harden the browser-only Remote Friend Code alpha until it survives a real friend demo. This is still the next product step because solo testing cannot prove the table experience, Discord/GW2 file sharing is friction, and guests should join from a browser without installing anything. Current state: `src/multiplayer/friend-code-session.js` defines tested friend-code/session primitives, internal-token separation, expiry/stop state, public projection, limits, and guest-safe relay message validation; the Cloudflare Worker relay is deployed for alpha use; Create Table can expose/connect a draft Remote Browser Link and Friend Code before the opening scene; draft codes are adopted into the real campaign/table/session when the table is created; Friends And Seats can create/copy/regenerate a Remote Friend Code, connect the host relay socket, register browser join requests and compact character drafts into the waiting room, return targeted approval to the browser, bridge approved browser guest snapshot/action/pass/vote/Table Talk messages into the existing host-local guest authority routes, and clean up local guest authority when the browser disconnects. The public browser page now has a first-pass playable table view after approval, auto-refreshes bounded guest-safe snapshots while connected, resends a pending join request when the host relay socket reconnects, and restores guest-facing friend-code/name/character draft fields after refresh. Rejoin requests can carry the previous party member as a preferred-seat hint, but the host-local waiting-room authority still decides seating. Remaining risk: real two-machine/off-LAN soak, durable rejoin-after-tab-close identity, expiry/idle UI, clearer errors, and no-mutation assertions around actual relay-to-authority handlers.
...
3. Continue moving recovery decisions out of `app/app.js` into TurnFlow, ProviderOrchestrator, CombatEngine, and multiplayer domain modules, especially where remote guest actions or relay messages could otherwise bypass table authority. Current state: remote relay join projection, active-code assertion, preferred-seat hint handling, guest authority payloads, snapshot queries, and snapshot compaction now live in `app/remote-relay-controller.js`; the renderer still owns the WebSocket event wiring and side-effect calls.
...
- [x] Host owns software-side campaign files, provider access/model calls, canon review, guest approvals, and persistence; the configured provider/model remains the DM voice at the table.
...
- [x] Model setup is reachable from Settings instead of being a first-class front-door flow.
...
- [x] Build first-pass Remote Friend Code MVP so off-LAN guests can join from a public browser page without downloading LoreKeeper. Current state: friend-code/session primitives and public projections exist; the Cloudflare Worker/Durable Object relay exists under `workers/relay`; the alpha relay is deployed at `https://lorekeeper-friend-relay.wirsingj.workers.dev`; public smoke checks pass for root, health, friendly host/table-code links, session lookup, host WebSocket active state, browser guest join-request relay, targeted approval relay, approved guest action relay without `sessionKey` leakage, and browser disconnect relay; the LoreKeeper host UI can create/copy a remote friend code during Create Table or Friends And Seats, connect the host relay socket, register relay join requests plus compact character drafts into the existing waiting-room flow, adopt setup-stage remote codes into the created table, tell the browser when the host seats them, send guest-safe snapshots, bridge browser Table Talk/action/pass/vote messages through existing local guest authority routes, send targeted Table Talk delivered/failed acknowledgements so browser messages do not feel lost while waiting on snapshots, preserve guest-facing reconnect form context and preferred-seat hints in browser storage, and clean up disconnected browser guests through the existing local disconnect route.
- [ ] Add remote relay message allowlist and no-mutation tests before expanding public relay play. Rejected/stale/oversized/unauthorized relay messages must not create provider calls, play-log entries, staged inputs, controller transfers, combat changes, recovery changes, or campaign mutations. Current state: guest-safe relay message kind/payload/host-only-field validation exists; `scripts/test-friend-relay.js` covers relay parser allowlist/rejection behavior; host approval/pending/denied/error/Table Talk acknowledgement messages require a connected target guest; `sessionKey` is allowed only on targeted `host.guest.approved`; remote join/action/pass/vote/Table Talk/snapshot requests are checked against the host's active friend code before entering local authority routes; `scripts/test-server-integration.js` proves stale relay-shaped guest action/vote requests do not mutate pending inputs, choice votes, Table Talk, or connection status; `app/remote-relay-controller.js` now has direct tests proving the renderer bridge prefers host-stored approved connection/party/session identity over browser-supplied identity, does not echo `sessionKey`, and rejects browser-initiated requests whose friend code no longer matches the active host session. Remaining gap: fuller no-mutation coverage for the async renderer WebSocket handlers themselves.
- [ ] Add Remote Friend Code UI polish: expiry/idle status, clearer disconnected/reconnecting state, and "experimental remote sharing" copy. Current state: Create Table exposes remote browser link/code controls for setup-stage guests with visible browser-link status and LAN as a fallback; Friends And Seats can start/stop sharing, copy code/link, regenerate a stale/leaked code, reconnect an active code when the relay socket is closed, show waiting guests through the same waiting-room list, connect to the configured relay, shows visible code expiry/idle/max-guest status plus experimental remote-sharing copy with host-side provider-key/Ollama/file safety, and shows connecting/reconnecting state in the share panel if the relay socket is not open. Remaining gap: real off-LAN feel testing.
- [x] Add public guest-page entry for friend code: enter code, display name, waiting approval, disconnected/rejected states, and browser-only play after approval. Current state: the Worker-served page renders a compact story/table view after approval with scene, recent story, choices, action, pass, refresh, and Table Talk controls, auto-refreshes guest-safe snapshots every 5 seconds while connected, and resends pending join requests after host relay reconnect; seat/character draft polish remains future work.
- [ ] Run first real two-machine/off-LAN playtest against the deployed relay. Current state: live relay-only WebSocket smoke proves host connect, guest join request, targeted approval, and guest action forwarding; full Electron host plus real browser guest table soak is still needed.
...
- [x] Guest-visible table snapshots redact nested host-only fields from generic campaign/message data, including connection/waiting secrets, invite/API tokens, provider prompts/settings, SQLite/local file paths, and raw provider/debug payloads.
...
- [x] Remote sharing exposes only guest-safe routes/messages by policy. Current state: an explicit allowlist exists for `/guest`, static guest app assets, and public guest table APIs; Share Table route categories include waiting-room, fixed invite, action/pass/vote, disconnect/rejoin, combat participation, and Table Talk routes, and are cross-checked against server route security tests; the friend-code relay has an explicit message-kind allowlist and rejects host-only fields, oversized payloads, untargeted approval/pending/denied/error messages, and `sessionKey` outside targeted approvals.
- [x] The host-facing share panel names the guest-safe boundary: preview, seat request, approved character actions, votes, pass, and Table Talk only; host settings, model setup, Ollama, files, and diagnostics stay host-side.
...
- [x] Browser guest join and in-app Join Table use aligned compact fields.
...
- [x] Main menu separates Continue Table, Create New Table, Join Table, and compact Settings from the in-campaign rails.
...
- [x] Split settings into App Settings and Table Settings as separate surfaces. Current state: the shared dialog component remains, but the front-door Settings surface exposes App and Models tabs, while in-table Friends And Seats, Diagnostics, and DM Recovery remain focused entry points.
- [x] Rename first-pass technical UI language so normal users see table, guest, model setup, Settings, and Troubleshooting instead of provider/SQLite/import/control-panel wording.
- [x] Reduce visible Settings controls to App and Models on the front door, with Friends, diagnostics, and recovery reached from relevant table/troubleshooting entry points.
- [ ] Rework the table screen into phase-aware action surfaces so users see what matters now instead of every system at once. Current state: the command deck now shows a TableSessionEngine-driven Now/Next cue, fresh-table Start Adventure, Seat Guest, and DM recovery actions live beside that cue, `app/table-action-controller.js` owns the main table CTA visibility policy, `app/table-focus-controller.js` maps phase to the surface that deserves attention, and the shell exposes `data-table-phase`/`data-table-focus`; the party, combat, notebook, and Table Talk rails now receive tested primary/supporting/quiet states, provider-status updates repaint the composer to keep phase copy aligned, Start Adventure hides after the host requests the opening for the current table session, pre-opening DM/AI companion nudges stay clickable only to explain the opening gate while Send Turn remains disabled until that opening has begun, backend starter seed campaigns are hidden from the front door, combat has an active-turn cue with controller responsibility plus legal actions, combat rows now show action/bonus/reaction/movement/concentration/resource state, and Inspect opens a focused recovery settings surface. Deeper combat action workflows and broader settings split work still need phase-specific behavior.
- [x] Make the front door feel closer to a game launcher: recent table, new table, join table, compact Settings, and no hidden last-table background. Current state: the primary grid now presents Continue Table, Create New Table, and Join Table as first-class choices, with model setup under Settings > Models.
- [x] Make Settings model setup first-run friendly enough for the portable LAN build. Current state: Model Setup now shows install/start/download/test readiness steps for Ollama and the selected model, with direct controller tests.
- [x] Add release guardrails for the portable distro: `npm run release:check`, temp-copy runtime smoke through `npm run smoke:portable`, local git hooks via `npm run hooks:install`, and guarded release tags through `npm run release:tag -- <tag>`. Current state: the zip includes `EXTRACT-FIRST.txt`, `release:check` verifies required zip entries, and `Open LoreKeeper.cmd` reports a clear extraction-first message if Windows runs it from a partial Temp-folder zip preview.
...
1. Open LoreKeeper on the host machine.
2. Choose Continue Table or Create New Table.
3. Confirm model setup is ready.
...
- 2026-06-18: Fresh-campaign playtest found AI companion Nudge was available before Start Adventure, which could send a model turn before the table's opening beat and fall into DM Recovery. Current fix: host Nudge buttons can remain clickable for explanation, but the command gate blocks provider generation before the opening; focused plus chaos UI harness coverage clicks the blocked button to prove it cannot start generation or recovery.
...
- 2026-06-19: Front-door saved-adventure projection moved out of `app/app.js` into `app/home-campaign-controller.js`, covering backend starter-campaign hiding, saved-adventure count copy, Continue/Delete enablement, selected campaign lookup, and safe delete-target projection. Verification: node syntax checks, `npm run test:engine`, `npm run build`, focused home baseline UI, and focused home delete UI passed.
- 2026-06-19: Scene intelligence/notebook projection moved out of `app/app.js` into `app/scene-notebook-controller.js`, using the same scene retrieval as DM context while hiding generated fallback scene shells from player-visible notebook records. Verification: node syntax checks, `npm run test:engine`, `npm run build`, focused visual-audit UI, and `npm run test:all` passed.
...
- 2026-07-25: Remote browser Table Talk now gets an explicit targeted host acknowledgement. The browser still echoes immediately, but the host now sends `host.tableTalk` delivered/failed messages so a player is not left wondering whether side chat vanished while waiting for the next bounded snapshot. Verification: `npm run test:friend-relay` and `npm run test:engine` passed.
- 2026-07-25: The public Remote Friend Code page now restores guest-facing reconnect context from browser storage: friend code, player name, compact character draft, and previous party member as a preferred-seat hint only. This reduces refresh/reconnect friction without introducing stored host authority secrets. Verification: `npm run test:friend-relay`, `npm run test:engine`, `npm run test:all`, and `npm run build` passed.
- 2026-07-25: Remote relay join requests now use the same active friend-code assertion as action/pass/vote/Table Talk/snapshot requests before they can enter the host-local waiting room. Join-request display name, client id, preferred-seat hint, and character draft projection now live in `app/remote-relay-controller.js` instead of renderer glue. Verification: `npm run test:engine` and `npm run test:friend-relay` passed.
