...
- [ ] Add Remote Friend Code UI polish: Regenerate, expiry/idle status, clearer disconnected/reconnecting state, and "experimental remote sharing" copy. Current state: Create Table exposes remote browser link/code controls for setup-stage guests; Friends And Seats can start/stop sharing, copy code/link, show waiting guests through the same waiting-room list, connect to the configured relay, shows experimental remote-sharing copy with code expiry, idle timeout, max guest count, and host-side provider-key/Ollama/file safety, and now gives the host a reconnect cue if the relay socket drops while sharing is active. Remaining gap: Regenerate and richer reconnect state in the share panel itself.
