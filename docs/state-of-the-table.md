...
1. Harden the browser-only Remote Friend Code alpha until it survives a real friend demo. This is still the next product step because solo testing cannot prove the table experience, Discord/GW2 file sharing is friction, and guests should join from a browser without installing anything. Current state: `src/multiplayer/friend-code-session.js` defines tested friend-code/session primitives, internal-token separation, expiry/stop state, public projection, limits, and guest-safe relay message validation; the Cloudflare Worker relay is deployed for alpha use; Create Table can expose/connect a draft Remote Browser Link and Friend Code before the opening scene; draft codes are adopted into the real campaign/table/session when the table is created; Friends And Seats can create/copy a Remote Friend Code, connect the host relay socket, register browser join requests into the waiting room, return targeted approval to the browser, and bridge approved browser guest snapshot/action/pass/vote/Table Talk messages into the existing host-local guest authority routes. The public browser page now has a first-pass playable table view after approval. Remaining risk: real two-machine/off-LAN soak, reconnect/disconnect behavior, expiry/idle UI, clearer errors, and no-mutation assertions around actual relay-to-authority handlers.
...
- [x] Build first-pass Remote Friend Code MVP so off-LAN guests can join from a public browser page without downloading LoreKeeper. Current state: friend-code/session primitives and public projections exist; the Cloudflare Worker/Durable Object relay exists under `workers/relay`; the alpha relay is deployed at `https://lorekeeper-friend-relay.wirsingj.workers.dev`; public smoke checks pass for root, health, friendly host/table-code links, session lookup, host WebSocket active state, browser guest join-request relay, targeted approval relay, and approved guest action relay without `sessionKey` leakage; the LoreKeeper host UI can create/copy a remote friend code during Create Table or Friends And Seats, connect the host relay socket, register relay join requests into the existing waiting-room flow, adopt setup-stage remote codes into the created table, tell the browser when the host seats them, send guest-safe snapshots, and bridge browser Table Talk/action/pass/vote messages through existing local guest authority routes.
