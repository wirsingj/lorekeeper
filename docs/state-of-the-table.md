...
171. UI chaos mode now runs seeded desktop/tabletop permutations for delayed DM generation, Table Talk during generation, cancel/retry, dialog churn, pre-lobby Add Crew uniqueness, provider recovery, AI companion combat locks, app-owned combat turns, common button affordances, and random non-destructive button-mashing across wizard, table, and combat phases.
...
200. Party-vote choice math now lives in `app/choice-vote-controller.js`: choice keys, vote counts, tie/leader state, host leading-choice projection, guest vote lookup, and selected-choice activity text are tested outside the renderer.
...
5. Party-vote collection now works for remote guests, clear leaders can be drafted by the host, ties are visible, and vote math is controller-tested. Final confirmation is still the normal Send Turn path rather than a dedicated modal.
