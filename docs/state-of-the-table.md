...
198. Play-log lifecycle wording for waiting, recovery, retries, dropped guest actions, and staged guest inputs now lives in `app/play-log-controller.js` with direct tests instead of being embedded in `app/app.js` DOM rendering.
