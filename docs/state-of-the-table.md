...
29. Remote Friend Code has a built-in alpha relay and first-pass browser guest table, but it is not release-calm yet. Browser close now sends guest disconnect through the relay and the host app cleans up through the local authority route. Friends And Seats can regenerate the remote friend code to invalidate a stale/leaked relay room, and browser-initiated relay requests are pinned to the current active friend code before the host bridge touches local guest authority. The remaining risk is live soak and hardening: reconnect/rejoin, richer expiry/idle UI, fuller async handler no-mutation tests, and real off-LAN Electron-host plus browser-guest play.
