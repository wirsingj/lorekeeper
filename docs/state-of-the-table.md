...
- 2026-07-25: Remote relay join requests now use the same active friend-code assertion as action/pass/vote/Table Talk/snapshot requests before they can enter the host-local waiting room. Verification: `npm run test:engine` and `npm run test:friend-relay` passed.
