...
- [x] Add public guest-page entry for friend code: enter code, display name, waiting approval, disconnected/rejected states, and browser-only play after approval. Current state: the Worker-served page renders a compact story/table view after approval with scene, recent story, choices, action, pass, refresh, and Table Talk controls, and auto-refreshes guest-safe snapshots every 5 seconds while connected; seat/character draft polish remains future work.
