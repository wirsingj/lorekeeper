...
- [ ] Add Remote Friend Code UI polish: expiry/idle status, clearer disconnected/reconnecting state, and "experimental remote sharing" copy. Current state: Create Table exposes remote browser link/code controls for setup-stage guests; Friends And Seats can start/stop sharing, copy code/link, regenerate a stale/leaked code, show waiting guests through the same waiting-room list, connect to the configured relay, shows visible code expiry/idle/max-guest status plus experimental remote-sharing copy with host-side provider-key/Ollama/file safety, and gives the host a reconnect cue if the relay socket drops while sharing is active. Remaining gap: richer reconnect state in the share panel itself and real off-LAN feel testing.
