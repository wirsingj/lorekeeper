...
- 2026-06-18: Choice selection parsing, edited-choice preservation, pending clicked-choice matching, and provider choice meta copy moved into `app/choice-vote-controller.js`, with architecture guards keeping that grammar out of `app/app.js`. Verification: `npm run test:engine`, `npm run build`, focused remote party-vote UI, and focused RP import UI passed.
