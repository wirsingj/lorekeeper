...
- The same Worker owns the public browser guest page for the remote alpha. That page should visually behave like a guest-safe LoreKeeper table surface, but it must remain a thin relay client: render guest-safe snapshots, send allowed guest messages, and avoid host/provider/settings/debug concepts.
