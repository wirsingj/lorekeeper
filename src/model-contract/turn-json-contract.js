...
const CONTROLLED_ACTOR_ACTION_VERBS = "(?:says?|asks?|replies?|answers?|shouts?|whispers?|signals?|gestures?|nods?|steps?|moves?|backs?|runs?|draws?|readies?|raises?|attacks?|strikes?|shoots?|casts?|touches?|grabs?|throws?|scans?|searches?|notices?|realizes?|thinks?|decides?|chooses?|insists?|refuses?|tightens?|tenses?|leans?|crouches?|flinches?|watches?|prepares?|locks?|locked|doesn'?t back down|does not back down|holds? (?:her|his|their) ground)";
...
export function buildTurnJsonPrompt({ campaign, contextPack, playerTurn, parsedMessage, options = {} } = {}) {
  ...
  return [
    ...
    "The app has already recorded user.inWorld as the player's table input. Do not echo it back as a party-member table entry.",
    "For normal responses, every table entry you create should use speaker 'DM', speakerId null, role 'dm'. Use a party member as speaker only when user.playerInputs contains that actor's submitted line/action.",
  ...
  const request = {
    ...
    meta: {
      ...
      instructionPriority: [
        ...
        "Do not output the latest player action as a party-member table entry; respond as DM to its consequences.",
...
function createResponseFormatSchema() {
  return {
    ...
    rules: [
      ...
      "The player's latest action is already visible in the play log. Do not mirror that action back as a table entry spoken by the party member.",
      "Use table[].speaker='DM' and table[].role='dm' for DM narration, NPC/world reaction, checks, consequences, and prompts. Use a party-member speaker only for explicit submitted user.playerInputs from that actor.",
      ...
      "In combat, respect context.combat.currentTurnId/current turn. Offer choices only for the active actor unless the app has supplied an already-resolved enemy/combat action to narrate.",
      "If context.combat.currentTurnId is an enemy, do not invent HP/resource/initiative changes. Enemy turn mechanics are app-owned; narrate only when the request includes a resolved action record.",
